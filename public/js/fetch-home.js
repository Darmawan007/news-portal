moment.locale("id");
const PushContain2 = (newestPosts) => {
    let i = 0;
    $("#post-carousel .swiper-slide").each(function () {
        ...
        <div class="w-full shadow-md relative">
            <a target="_blank" href="${newestPosts[i]?.link}">
                <img src="${
                    newestPosts[i]?.thumbnail
                }" alt="" class="md:h-96 h-64 w-full object-cover font-semibold brightness-50" />
            </a>
            <div class="absolute inset-x-0 bottom-0 md:p-4 p-3 bg-gradient-to-r from-blue-800 flex flex-col">
                <span class="text-white md:text-2xl text-base line-clamp-2">${
                    newestPosts[i]?.title
                }</span>
                <span class="text-gray-200 md:text-sm text-[11px] mt-2 line-clamp-2">${
                    newestPosts[i]?.description
                }</span>
            </div>
            <div class="absolute top-0 left-0 md:p-4 p-3 flex items-center md:gap-x-4 gap-x-3">
                <div>
                    <div class="md:w-10 md:h-10 w-8 h-8 rounded-full bg-white">
                        <img src="${
                            newestPosts[i]?.source?.image
                        }" alt="" class="md:w-10 md:h-10 w-8 h-8 rounded-full object-contain" />
                    </div>
                </div>
                <div>
                    <div class="text-white md:text-base text-sm line-clamp-1">${
                        newestPosts[i]?.source?.title
                    }</div>
                    <div class="text-white md:text-xs text-[11px]">${moment(
                        newestPosts[i]?.pubDate
                    ).fromNow()}</div>
                </div>
            </div>
        </div>`;
    ...
    const postSide = [0, 1, 2, 3].map(() => {
        ...
        <div class="w-full shadow-md relative">
            <a target="_blank" href="${newestPosts[i]?.link}">
                <img src="${
                    newestPosts[i]?.thumbnail
                }" alt="" class="md:h-[184px] h-60 w-full object-cover font-semibold brightness-50" />
            </a>
            <div class="absolute inset-x-0 bottom-0 md:p-2 p-3 bg-gradient-to-r from-blue-800 flex flex-col">
                <span class="text-white md:text-base text-base line-clamp-2">${
                    newestPosts[i]?.title
                }</span>
                <span class="text-gray-200 md:text-sm text-[11px] mt-2 line-clamp-2 md:hidden block">${
                    newestPosts[i]?.description
                }</span>
            </div>
            <div class="absolute top-0 left-0 md:p-2 p-3 flex items-center md:gap-x-2 gap-x-3">
                <div>
                    <div class="w-8 h-8 rounded-full bg-white">
                        <img src="${
                            newestPosts[i]?.source?.image
                        }" alt="" class="w-8 h-8 rounded-full object-contain" />
                    </div>
                </div>
                <div>
                    <div class="text-white text-sm line-clamp-1">${
                        newestPosts[i]?.source?.title
                    }</div>
                    <div class="text-white text-[11px]">${moment(
                        newestPosts[i]?.pubDate
                    ).fromNow()}</div>
                </div>
            </div>
    ...
    const otherPosts = newestPosts.slice(i, i + 20).map(
        (newestPost) => `
        ...
        <div class="flex gap-x-4 relative">
            <div>
                <div class="h-[72px] w-32 md:w-40 md:h-24">
                    <img src="${
                        newestPost?.thumbnail
                    }" alt="" class="h-full w-full object-cover" />
                </div>
            </div>
            <div>
                <div class="md:text-base text-sm line-clamp-2 font-semibold -mt-1 hover:text-blue-700">${
                    newestPost?.title
                }</div>
                <div class="md:text-xs text-[10px] line-clamp-2 text-gray-400">${moment(
                    newestPost.pubDate
                ).fromNow()}</div>
                <div class="md:text-xs text-[10px] md:line-clamp-2 line-clamp-1 md:mt-1 mt-0.5 ">${
            ...
            <div class="absolute md:top-6 md:left-14 top-3 left-9 flex items-center justify-center md:gap-x-2 gap-x-3">
                <div>
                    <div class="w-12 h-12 rounded-full bg-white shadow-lg">
                        <img src="${
                            newestPost?.source?.image
                        }" alt="" class="w-12 h-12 rounded-full object-contain" />
                    </div>
                </div>
            </div>
        </div>
...
const mergeArray = (...array) => {
    ...
    return [...new Array(max).fill(0)].flatMap((_, i) =>
        array.map((arr) => arr[i]).filter((v) => v)
    );
};
...
async function fetchData2() {
    ...
    PushContain2(
        mergeArray(
            transformPost2(cnnPosts),
            transformPost2(antaraPosts),
            transformPost2(cnbcPosts)
        )
    );
}
...
$(document).ready(function () {
    $("#source-carousel .swiper-slide").each(function (i) {
        ...
        <div class="shadow-md md:h-28 h-20 md:w-28 w-20 rounded-full overflow-hidden flex items-center">
            <a href="./source?source=${sources[i]?.name}">
